...
while true; do
  ...
  FAILED_STATUS=$(kubectl get job -n "$BACKUP_NAMESPACE" "$BACKUP_JOB_NAME" -o jsonpath='{.status.failed}')
...
# Create the restore jobs in mariadb-standalone and cloud instances
