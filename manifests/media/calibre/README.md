...
4. Replace the default `api_endpoint=https://storeapi.kobo.com` with the URL and token generated in the user's account section. Will look something like `https://books.laskonet.com/kobo/<LongToken>`
...
Also included is a book downloader that integrates with Calibre-Web-Automated, which uses the [Calibre Web Automated Book Downloader](https://github.com/calibrain/calibre-web-automated-book-downloader) to download books. Downloaded books are automatically added to the Calibre library. Accessible via https://book-dl.laskonet.com

Most configuration is done via my [custom Helm chart](/helm/baseline)


# Troubleshooting
## Kobo sync not wokring
1. Check that the JSON is populating properly by going to https://books.laskonet.com/kobo/<token>/v1/library/sync
2. Make sure that `Proxy unknown requests to Kobo Store` is selected under Settings - Edit Basic Configuration - Feature Configuration - Enable Kobo Sync