...
* A `*.laskonet.com` wildcard certificate for all web services
* A `*.dns.laskonet.com` wildcard certificate for [AdGuard Home](/manifests/apps/adguard) private DNS

The `laskonet.com` domain is hosted on [Cloudflare](https://dash.cloudflare.com/login) and utilizes the [Cloudflare DNS01 challenge provider](https://cert-manager.io/docs/configuration/acme/dns01/cloudflare/) for proving ownership and approving certificates.
