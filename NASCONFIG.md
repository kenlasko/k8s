...
```
#! /bin/bash
# This script is used to backup the NAS to the BACKUPPC backup server using rsync
# Assumes that the NAS is mounted at /mnt/media and the BACKUPPC server is mounted at /media/backup
...
rsync -aP --no-perms --no-owner --no-group --delete /mnt/backup/apps/ /media/other/backup/apps
if [ "$?" -ne 0 ]; then
    echo "AppData backup failed";
fi

rsync -aP --no-perms --no-owner --no-group --delete /mnt/backup/vol/ /media/other/backup/vol
if [ "$?" -ne 0 ]; then
